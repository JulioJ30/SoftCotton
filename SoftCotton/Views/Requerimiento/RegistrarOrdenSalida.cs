...
{
    public partial class RegistrarOrdenSalida : Form
    {
        ...
        private void RegistrarOrdenProduccion_Load(object sender, EventArgs e)
        {
            CbAlmacen.DataSource = _mantenimientoBL.SetDatatable(62, UserApplication.USUARIO, "", 0, 0);
            CbAlmacen.DisplayMember = "Descripcion";
            CbAlmacen.ValueMember = "codAlmacen";
        ...
        private void btnEliminarOS_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult resultadoMensaje = ResponseMessage.MessageQuestion("¿Esta seguro en eliminar la orden de producción N° " + txtCodigo.Text.Trim() + " ?");
                ...
                if (resultadoMensaje == DialogResult.OK)
                {
                    ...
                    Response responseGeneral = _ordenServicioBL.SetOrdenSalida(parametro, CbAlmacen.SelectedValue.ToString());
            ...
            {
                ResponseMessage.Message("Ingrese un código de Orden producción válido.", "WARNING");
            }
        }
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtCodigo.Text))
            {
                ...
                {
                    ResponseMessage.Message("No existe orden de producción", "WARNING");
                }
            }
            else
            {
        ...
        private void dgvOSDetalle_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (cbxTipoSalida.SelectedIndex == 0)
            {
            ...
            else if (e.Control && e.KeyCode == Keys.D)
            {
                if (dgvOSDetalle.CurrentRow != null)
                {
                    ...
                    if (resultadoMensaje == DialogResult.OK)
                    {
                        ...
                        Response respuesta = _ordenServicioBL.SetOrdenSalidaDetalle(osDetParam);
        ...
        private void txtCodigo_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                if (e.KeyChar == (char)13)
                {
                    if (!string.IsNullOrEmpty(txtCodigo.Text))
                    {
                        int idOrdenSalida = Convert.ToInt32(txtCodigo.Text);
                        Limpiar();
                        BuscarOrdenSalida(Empresa.ID_EMPRESA, idOrdenSalida);
                    }
                    else
                    {
                        ResponseMessage.Message("Ingrese un código de Orden de producción", "INFORMATION");
                    }
        ...
        private void CalcularMontosFinales()
        {
            ...
            if (cbxTipoSalida.SelectedIndex == 1)
            {
                // COMPRA NACIONAL
            ...
            else if (cbxTipoSalida.SelectedIndex == 2)
            {
                // COMPRA IMPORTADO
        ...
        public void Procesar(SetOSParam parametro)
        {
            ...
            if (responseGeneral.idResponse > 0)
            {
                ...
                if (parametro.opcion == 1)
                {
                    ...
                    Response respuestaFirma = _ordenServicioBL.SetOrdenSalidaAprobacion(parametroAprobacion, tipoSalida);
        ...
        private void ProcesarDetalle(int idEmpresa, int idOC, int opcion, string tipoCompra)
        {
            ...
            foreach (DataGridViewRow row in dgvOSDetalle.Rows)
            {
                if (estadoProceso && (contador < dgvOSDetalle.Rows.Count))
                {
                    ...
                    Response respuesta = _ordenServicioBL.SetOrdenSalidaDetalle(ocDetParam);
        ...
        private void BuscarOrdenSalida(int idEmpresa, int idOrdenSalida)
        {
            GetOS7_OSCabXCodigo ocCab = _ordenServicioBL.Get26_OCCabXCodigo(idEmpresa, idOrdenSalida);

            txtCodigo.Text = idOrdenSalida.ToString();
            ...
            if (ocCab.tipoCompra == "P")
            {
                cbxTipoSalida.SelectedIndex = 1;
            }
            else if (ocCab.tipoCompra == "E")
            {
                cbxTipoSalida.SelectedIndex = 2;
            }
            ...
            ListarFirmantes(Empresa.ID_EMPRESA, idOrdenSalida);
            ListarDetalle(Empresa.ID_EMPRESA, idOrdenSalida);
        }


        private void ListarDetalle(int idEmpresa, int codigoOC)
        {
            List<GetOS8_OSDetXCodigo> ocDets = _ordenServicioBL.Get27_OSDetXCodigo(idEmpresa, codigoOC);
        ...
        {
            if (cbxTipoSalida.SelectedIndex != 0)
            {
                CalcularMontosFinales();
            }
            else
